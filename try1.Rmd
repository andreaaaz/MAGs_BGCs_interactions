...
bgc_4 <- c(0, 0, 1, 0, 1, 0, 1, 0, NA, 0)

gcfs_by_sites <- data.frame(station, bgc_1, bgc_2, bgc_3, bgc_4)

mag_1 <- c(2, 0, 1, 0, 0, 0, 3, 1, 0, 0) # co-occurrence 
mag_2 <- c(1, 3, 0, 0, 0, 1, 0, 0, 0, 1)
mag_3 <- c(0, 1, 1, 0, 0, 2, NA, 0, 0, 0) # co-exclusion
mag_4 <- c(2, 1, 0, 0, 0, 2, NA, 0, 1, 0)

motus_by_sites <- data.frame(station, mag_1, mag_2, mag_3, mag_4)
...
```{r stats}
n_sites <- length(unique(motus_by_sites$station))

stats_exlcu <- co_exclu %>%
  mutate(
    p_bgc  = (bgc_sites / n_sites) * (1 - (mag_sites / n_sites)),
    p_mags = (mag_sites / n_sites) * (1 - (bgc_sites / n_sites))
  )

stats_ocur <- co_occur %>%
  mutate(
    p = (total_sites / n_sites) * (total_sites / n_sites)
  )
