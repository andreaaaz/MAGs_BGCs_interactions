...
for (col1 in colnames(motus_by_sites)) {
  ...
  for (col2 in colnames(gcfs_by_sites)) {
    ...
    # bind col1 and col2 by station, because some bgcs aren't in all the stations, 
...
cont <- (which(colnames(motus_by_sites) == col1)) * (ncol(gcfs_by_sites)-1) + (which(colnames(gcfs_by_sites) == col2))
