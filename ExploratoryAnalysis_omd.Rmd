...
# Unir estacion con profundidad para definir sitios
meta_mags <- meta_mags %>%
  mutate(sites = paste(station, sprintf("%.2f", depth), sep = "_"))

write.csv(meta_mags, "~/MAGs_BGCs_interactions/metadata.csv", row.names = FALSE)

meta_bgcs<- meta_bgcs %>%
  left_join(meta_mags %>% select(Genome, sites), by = "Genome")

write.csv(meta_bgcs, "~/MAGs_BGCs_interactions/bgcs_metadata.csv", row.names = FALSE)
...
# overrall checkm for mags

meta_mags %>% 
  ggplot( aes(x=CheckM_Completeness)) +
    stat_bin(breaks=seq(0,100,4), fill="#69b3a2", color="#e9ecef", alpha=0.9) +
    ggtitle("Checkm completeness") +
    theme_minimal()

meta_mags %>% 
  ggplot( aes(x=CheckM_Contamination)) +
    stat_bin(breaks=seq(0,10,0.2), fill="#69b3a2", color="#e9ecef", alpha=0.9) +
    ggtitle("Checkm completeness") +
    theme_minimal()

ggplot(meta_mags, aes(x = CheckM_Completeness, 
               y = CheckM_Contamination, 
               color = CheckM_Strain_heterogeneity)) +
...
qc_mags <- meta_mags %>%
  group_by(sites) %>%
  summarise(
    n_mags = n_distinct(Genome),
    n_scaffolds = sum(X._scaffolds, na.rm = TRUE),
    n_families = n_distinct(family, na.rm = TRUE),
    n_genus = n_distinct(genus, na.rm = TRUE),
    n_motus = n_distinct(mOTUs_Species_Cluster, na.rm = TRUE),
    .groups = "drop"
  )
...
qc_bgcs <- meta_bgcs %>%
  group_by(sites) %>%
  summarise(
    n_bgcs = n_distinct(BGC),
    n_gcfs = n_distinct(gcf, na.rm = TRUE),
    n_gccs = n_distinct(gcc, na.rm = TRUE),
    .groups = "drop"
  )

# Combine
qc_sites <- qc_mags %>%
  full_join(qc_bgcs, by = "sites") %>%
  mutate(
    across(
      c(n_mags, n_scaffolds, n_families, n_genus, n_motus,
        n_bgcs, n_gcfs, n_gccs),
      ~ replace_na(.x, 0)
    )
  )
...
qc_sites %>% 
  ggplot( aes(x=n_mags)) +
    stat_bin(breaks=seq(0,100,1), fill="#69b3a2", color="#e9ecef", alpha=0.9) +
    ggtitle("Number of MAGs in sites") +
    theme_minimal() 

qc_sites <- qc_sites %>%
  full_join(meta_sites, by = "sites")

ggplot(qc_sites, aes(x = temperature_..C., y = n_mags)) +
  geom_point(alpha = 0.7, size = 1) +
  theme_minimal()
ggplot(qc_sites, aes(x = depth, y = n_mags)) +
  geom_point(alpha = 0.7, size = 1) +
  theme_minimal()


# add number of mags in the meta sites info
...
write.csv(meta_sites, "~/metadata/meta_sites.csv", row.names = FALSE)
